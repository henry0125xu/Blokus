...
int main(int argc,char* argv[]){
    ...
    while(!feof(LogF)){
        char c=fgetc(LogF);
        if(c==opp_color) flag=true;
        if(flag){
            if(c=='-'){
                opp_log[k].x=(step_log[i-1])-48;
                opp_log[k].y=(step_log[i+1])-48;
                k++;
                flag=false;
            }
        }
    ...
    //if round > 1
        ...
        do{
            ...
            do{
                //range = 10~20
                choice=(rand()%11+9);
            }while(( left_pattern[choice]=='0' || dead(choice) ) && (clock()-start<800) );
            while(( left_pattern[choice]=='0' || dead(choice) ) &&  ((clock()-start)<1500) &&  ((clock()-start)>=800)){
                //range = 3~21
                choice=(rand()%19)+2;
            }
            //chance of degree
            r=0;
            while(!putted && (r<10) && ((clock()-start)<1500)){
                //random degree
                rotation=(rand()%4);
                reflection=(rand()%2);
                //consider oppside
                ...
                }while(!putted && (cnt>0) && ((clock()-start)<800));
                //from inside to outside
                cnt=p*p;
                while(!putted && (cnt>0) && ((clock()-start)<1500) && ((clock()-start)>=800)){
                    x=rand()%p+q;
                    y=rand()%p+q;
                    cnt--;
                    putted=legality(choice,x,y,rotation,reflection,false);
            ...
            p+=2;
            q--;
        }while(!putted && p<=14 && ((clock()-start)<1500)) ;
        //brute force
        if(!putted){
            for(i=0;i<14;i++){
                for(j=0;j<14;j++){
