        ...
        while(!putted && p<=14) {
            if(game_round<=10){
                do{
                    //10~21
                    choice=(rand()%12+9);
                }while(( left_pattern[choice]=='0' || dead(choice) ) && game_round<=10 &&  clock()-start<2000);
            }
            else{
                do{
                //all
                    choice=(rand()%21);
                }while(( left_pattern[choice]=='0' || dead(choice) ) &&  clock()-start<2000);
            }
            //from inside to outside
            i=0;
            while(!putted && i<8 && clock()-start<2000){
                cnt=p*p;
                //random degree
                rotation=(rand()%4);
                reflection=(rand()%2);
